...
void lambdaPolAnal() {
    ...
    Int_t nFile = 50; //number of input file
    Int_t enhancedValue = 500; //number of enhanced lambdass
    ...
    for(Int_t iFile = 1; iFile < nFile; ++iFile){// Uncomment the loop if it is needed for proton production
        ...
        simulate_lambda_decays(TString::Format("%surqmd_xexe_2.87gev_mf_6195240_%i.mcini.root", pathIn.Data(), iFile), TString::Format("%s%s",pathOut.Data(),fileOut.Data()), iFile, enhancedValue);
    }
